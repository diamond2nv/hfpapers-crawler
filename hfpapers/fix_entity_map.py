...
def verify_map(name: str, arxiv_id: str, expected_title: str, db) -> dict:
    ...
    row = db.execute("SELECT title, authors, doi, journal_ref FROM arxiv_meta WHERE arxiv_id = ?", (arxiv_id,)).fetchone()
    if not row:
...
CORRECTED_MAP = {
    ...
    "neural-stagger": ("2302.10255", "NeuralStagger: Accelerating Physics-constrained Neural PDE Solver with Spatial-temporal Network"),
    ...
    "gnp-geometric-neural-operator": ("2404.10843", "Geometric Neural Operators (GNPs) for Data-Driven Deep Learning of Solutions to Real-World PDEs"),
    ...
    "multiscale-neural-operator": ("2510.16071", "MNO: Multiscale Neural Operator for 3D Computational Physics"),
    ...
    # AI agent papers
    "ai-scientist-v2": ("2501.17822", "The AI Scientist-v2: Workshop-Level Automated Scientific Discovery via LLM Agent Collaboration"),
